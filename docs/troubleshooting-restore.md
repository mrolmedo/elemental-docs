...
Before you initiate a restore, you need to copy `/var/lib/rancher/agent/rancher2_connection_info.json` from the elemental node to a node where you have `kubectl` access to the Rancher upstream cluster. This is required in order to run the below script successfully.
